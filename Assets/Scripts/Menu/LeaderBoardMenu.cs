...
public class LeaderboardsMenu : Panel
{
    ...
    public async void AddScoreAsync(int score)
    {
        ...
        try
        {
            var playerEntry = await LeaderboardsService.Instance.AddPlayerScoreAsync("Test_Leaderboard", score);
    ...
    private async void LoadPlayers(int page)
    {
        ...
        try
        {
            ...
            var scores = await LeaderboardsService.Instance.GetScoresAsync("Test_Leaderboard", options);
            ClearPlayersList();
