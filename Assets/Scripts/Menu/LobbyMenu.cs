...
public class LobbyMenu : Panel
{
    ...
    public async void CreateLobby(string lobbyName, int maxPlayers, bool isPrivate, string map)
    {
        ...
        try
        {
            ...
            options.Player.Data.Add("team", new PlayerDataObject(visibility: PlayerDataObject.VisibilityOptions.Public, value: "Team 1"));
    ...
    private async void UpdatePlayerTeam(string playerId, string team)
    {
        try
        {
            UpdatePlayerOptions options = new UpdatePlayerOptions();
            options.Data = new Dictionary<string, PlayerDataObject>();
            options.Data.Add("team", new PlayerDataObject(visibility: PlayerDataObject.VisibilityOptions.Public, value: team));
    ...
    private async void SwitchReady()
    {
        ...
        {
            Debug.Log("Player in lobby: " + p.Id);
        }
    ...
    private async void SwitchTeam()
    {
        ...
        try
        {
            // Kiểm tra lobby và danh sách người chơi
            if (lobby == null)
            {
            ...
            {
                Debug.LogError("Players list in lobby is null.");
                return;
            }

            if (lobby.Players.Count == 0)
            {
                Debug.LogWarning("Players list is empty.");
                return;
            }
            ...
            if (player == null)
            {
                Debug.LogError("Không tìm thấy người chơi hiện tại với ID: " + AuthenticationService.Instance.PlayerId);
                return;
            }

            if (!player.Data.ContainsKey("team"))
            {
                Debug.LogError("Thông tin đội không tồn tại cho người chơi hiện tại.");
                return;
            }

            var currentTeam = player.Data["team"].Value;
            Debug.Log("Đội hiện tại của người chơi: " + currentTeam);

            // Chuyển đội
            string newTeam = (currentTeam == "Team 1") ? "Team 2" : "Team 1";
            int newTeamCount = lobby.Players.Count(p => p.Data.ContainsKey("team") && p.Data["team"].Value == newTeam);
            if (newTeamCount < 5)  // Mỗi đội chỉ có tối đa 5 người
            {
                ...
                Debug.Log("Chuyển thành công sang " + newTeam);
                LoadPlayers();
            }
            else
            {
                Debug.LogWarning("Đội mới đã đầy, không thể chuyển.");
            }
        }
        catch (Exception exception)
        {
            Debug.LogError("Lỗi khi chuyển đội: " + exception.Message);
        }
        finally
        {
